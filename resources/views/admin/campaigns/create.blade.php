...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl border border-warm-100 shadow-sm p-6" x-data="{ bgColor: '{{ old('bg_color', '#e97a1e') }}', textColor: '{{ old('text_color', '#ffffff') }}', content: '{{ old('content', '') }}' }">
        {{-- Live Preview --}}
        <div class="mb-6">
            ...
            <div class="py-2.5 px-4 rounded-lg text-sm font-medium text-center" :style="'background-color:' + bgColor + '; color:' + textColor">
                <span x-text="content || 'Teks banner akan muncul di sini'"></span>
            </div>
        </div>

        <form action="{{ route('admin.campaigns.store') }}" method="POST" class="space-y-5">
            @csrf

            <div>
                <label for="campaign-title" class="block text-sm font-medium text-warm-700 mb-1.5">Judul (Internal)</label>
                <input type="text" id="campaign-title" name="title" value="{{ old('title') }}" required maxlength="255"
                       ...
                       placeholder="Promo Batch #5">
                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="campaign-content" class="block text-sm font-medium text-warm-700 mb-1.5">Teks Banner</label>
                <input type="text" id="campaign-content" name="content" x-model="content" value="{{ old('content') }}" required maxlength="500"
                       class="w-full border border-warm-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                       placeholder="Gunakan kode MAMAYA10 untuk diskon 10%!">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" id="campaign-sort" name="sort_order" value="{{ old('sort_order', 0) }}" min="0"
                           class="w-full border border-warm-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                </div>
            </div>

            <div>
                <label for="campaign-link" class="block text-sm font-medium text-warm-700 mb-1.5">Link URL <span class="text-warm-400">Opsional</span></label>
                <input type="url" id="campaign-link" name="link_url" value="{{ old('link_url') }}" maxlength="255"
                       class="w-full border border-warm-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                       placeholder="https://...">
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="campaign-start" class="block text-sm font-medium text-warm-700 mb-1.5">Mulai Tayang <span class="text-warm-400">Opsional</span></label>
                    <input type="datetime-local" id="campaign-start" name="start_date" value="{{ old('start_date') }}"
                           class="w-full border border-warm-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                </div>
                <div>
                    <label for="campaign-end" class="block text-sm font-medium text-warm-700 mb-1.5">Selesai Tayang <span class="text-warm-400">Opsional</span></label>
                    <input type="datetime-local" id="campaign-end" name="end_date" value="{{ old('end_date') }}"
                           class="w-full border border-warm-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" id="campaign-active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-brand-500 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-brand-600 transition-colors shadow-sm">Simpan Campaign</button>
